...
let count = 0;
const toastState = { toasts: [] as ToasterToast[], listeners: new Set<() => void>() };

function genId() { return `${++count}`; }

function addToast(toast: Omit<ToasterToast, 'id'>) {
  const id = genId();
  toastState.toasts = [{ ...toast, id, open: true }, ...toastState.toasts].slice(0, TOAST_LIMIT);
...
export function toast(props: Omit<ToasterToast, 'id'>) { addToast(props); }
