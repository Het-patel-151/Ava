...
import { synthesizeSpeech, streamChatCompletion, transcribeAudio } from './ai.service';
...
export const handleVoiceEvents = (io: SocketServer, socket: AuthenticatedSocket) => {
  ...
  socket.on('voice:audio', async (audioData: { buffer: ArrayBuffer; conversationId?: string }) => {
    try {
      ...
      let systemPrompt = 'You are AVA, a helpful and friendly AI voice assistant. Keep responses concise (2-3 sentences max) and conversational.';
      ...
      if (audioBuffer.length > 0) {
        ...
        // TTS unavailable — frontend will show text only
        socket.emit('voice:text_only', { text: fullResponse });
      }
  ...
  socket.on('voice:stop', () => socket.emit('voice:stopped', { message: 'Voice session ended' }));
};
